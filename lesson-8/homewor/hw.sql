...
SELECT DISTINCT customerid FROM Orders WHERE YEAR(orderdate) = 2023 EXCEPT SELECT DISTINCT customerid FROM Invoices;
...
Select year(orderdate) as orderyear, avg(order_amount) as averageamount from orders group by year(orderdate);
...
SELECT productname,
       CASE 
           ...
           ELSE 'High'
       END AS price_group FROM Products;
...
Select ProductId, sum(scale_amount) as Totalsales from Sales Group by ProductId;
...
SELECT producId FROM Products INTERSECT SELECT productId FROM Products_Discounted;

Hard-Level Tasks

19. Using Invoices table, show top 3 customers with the highest total invoice amount. Return CustomerID and Totalspent.
SELECT customerid, SUM(invoiceamount) AS TotalSpent
FROM Invoices GROUP BY customerid ORDER BY TotalSpent DESC LIMIT 3;
SELECT TOP 3 customer_id, 
       SUM(invoice_amount) AS TotalSpent
FROM Invoices
GROUP BY customer_id
ORDER BY TotalSpent DESC;

20. Find product ID and productname that are present in Products but not in Products_Discounted.
SELECT product_id, product_name FROM Products WHERE product_id NOT IN (SELECT product_id FROM Products_Discounted
